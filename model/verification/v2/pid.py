...
def check_pid(nl: Dict[str, Any]) -> Dict[str, Any]:
    ...
    * ``PASS``          — all 3 distinct parallel branches present and every path
    ...
    if amp is None or not summing:
        # Dual-review: do NOT auto-PASS here. Absence of the expected node does
        # not prove PID is inapplicable — it can also mean a malformed/unrecognized
        # intended PID, so a vacuous PASS would be fail-open. Keep INDETERMINATE
        # (specialist) unless we can positively prove no PID intent exists.
